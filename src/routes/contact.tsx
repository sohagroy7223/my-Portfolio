...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Get in touch with Sohag Roy for projects, freelance, or collaborations." },
...
function ContactPage() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    const mailto = `mailto:sohagroy7223@gmail.com?subject=${encodeURIComponent(form.subject)}&body=${encodeURIComponent(body)}`;
  ...
  return (
    <PageTransition>
      <Section eyebrow="Contact" title="Let's talk" subtitle="Drop a note and I'll get back to you soon.">
        <div className="grid gap-8 md:grid-cols-5">
            ...
            <div className="glass rounded-2xl p-6">
              ...
              <ul className="mt-4 space-y-4 text-sm">
                <li className="flex items-start gap-3">
                  <span className="grid h-9 w-9 place-items-center rounded-lg bg-primary/10 text-primary"><FiMail /></span>
                  <div>
                    <div className="font-medium">Email</div>
                    <a href="mailto:sohagroy7223@gmail.com" className="text-muted-foreground hover:text-foreground">sohagroy7223@gmail.com</a>
                  </div>
                </li>
                <li className="flex items-start gap-3">
                  <span className="grid h-9 w-9 place-items-center rounded-lg bg-secondary/15 text-secondary"><FiPhone /></span>
                  <div>
                    <div className="font-medium">Phone</div>
                    <a href="tel:+8801311976179" className="text-muted-foreground hover:text-foreground">+880 1311-976179</a>
                  </div>
                </li>
                <li className="flex items-start gap-3">
                  <span className="grid h-9 w-9 place-items-center rounded-lg bg-accent/15 text-accent-foreground"><FiMapPin /></span>
                  <div>
              ...
              <div className="mt-6">
                ...
                <div className="mt-3 flex gap-3">
                  {[
                    ...
                    <a key={i} href={href} target="_blank" rel="noreferrer" className="grid h-10 w-10 place-items-center rounded-xl border border-border transition-all hover:-translate-y-0.5 hover:bg-primary hover:text-primary-foreground">
                      <I />
                    </a>
                  ))}
                </div>
          ...
          <Reveal delay={0.1} className="md:col-span-3">
            <form onSubmit={submit} className="glass space-y-4 rounded-2xl p-6">
              ...
              >
                {sent ? (<><FiCheck /> Sent!</>) : (<><FiSend /> Send Message</>)}
              </button>
            </form>
          </Reveal>
        </div>
